	...
	<div id="wrap">
		...
		<article>
			...
			<table id="notice">
				...
				<tr>
					<td colspan="6">
						받는 사람 : <input type="text" name="o_r_name" value="${oDTO.o_r_name }"><br>
						연락처 : <input type="text" name="o_r_phone" value="${oDTO.o_r_phone }"><br>
						배송지 주소 : <input type="text" name="o_r_addr1"><br>
						상세 주소 : <input type="text" name="o_r_addr2"><br>
			...
			<input type="hidden" name="trade_num" value="${oDTO.o_trade_num }">
				<hr>
					<select name="status">
						...
						<option value="0"
							<c:if test="${oDTO.o_status == 0 }">
								selected
							</c:if>
						>대기중</option>
						<option value="1"
							<c:if test="${oDTO.o_status == 1 }">
								selected
							</c:if>
						>발송준비</option>
						<option value="2"
							<c:if test="${oDTO.o_status == 2 }">
								selected
							</c:if>
						>발송완료</option>
						<option value="3"
							<c:if test="${oDTO.o_status == 3 }">
								selected
							</c:if>
						>배송중</option>
						<option value="4"
							<c:if test="${oDTO.o_status == 4 }">
								selected
							</c:if>
						>배송완료</option>
						<option value="5"
							<c:if test="${oDTO.o_status == 5 }">
								selected
							</c:if>
						>주문취소</option>
					</select>
